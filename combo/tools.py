...
class SimplifyRule:
    def xiRule(exp):
        ...
        res=0
        with tqdm(total=len(expArgs), desc="xiRule caculating") as pbar:
            for i in expArgs:
    ...
    def natRule(exp):
        expArgs=exp.args#表达式按照加减分成元组不同元素
        res=0 
        with tqdm(total=len(expArgs), desc="natRule caculating") as pbar:
            for i in expArgs:
                elemArgs=i.args#表达式按照乘分成元组的不同元素
                subexp=1
                for j in elemArgs:
...
class Filter:
    def filterbody(terms,bodyType):
        if (type(terms)!=type(X[{},{}]+Y[{},{}])):#input terms like A[{},{}]*B[{},{}] or A[{},{}]
            ...
            res=0
            with tqdm(total=len(terms.args), desc="filtering") as pbar:
